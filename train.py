...
from utils import tensor2array, save_checkpoint, compute_depth, compute_pose_with_inv
...
parser.add_argument('--with-auto-mask', type=int,  default=0, help='with the mask for stationary points')
...
parser.add_argument('--padding-mode', type=str, choices=['zeros', 'border'], default='zeros',
                    help='padding mode for image warping: zeros null gradients outside target image, border only null gradients outside x or y')
parser.add_argument('--with-gt', action='store_true', help='use ground truth for validation. See data/kitti_raw_loader.py for an example')
...
def build_optimizer(args, disp_net, pose_net):
...
def main():
    ...
    disp_net, pose_net = build_models(args)
    print('=> setting adam solver')
    optimizer = build_optimizer(args, disp_net, pose_net)
...
def train(args, train_loader, disp_net, pose_net, optimizer, epoch_size, logger, train_writer):
    ...
    for i, (tgt_img, ref_imgs, intrinsics, intrinsics_inv) in enumerate(train_loader):
        ...
        loss_1, loss_3 = compute_photo_and_geometry_loss(
            tgt_img_forDepth, ref_imgs_forDepth, intrinsics, tgt_depth, ref_depths,
...
def validate_without_gt(args, val_loader, disp_net, pose_net, epoch, logger, output_writers=[]):
    ...
    for i, (tgt_img, ref_imgs, intrinsics, intrinsics_inv) in enumerate(val_loader):
        ...
        tgt_depth = [1 / disp_net(tgt_img_forDepth)]
        ref_depths = []
        for ref_img in ref_imgs_forDepth:
            ref_depths.append([1 / disp_net(ref_img)])

        if log_outputs and i < len(output_writers):
            if epoch == 0:
                output_writers[i].add_image('val Input', tensor2array(tgt_img_forDepth[0]), 0)
