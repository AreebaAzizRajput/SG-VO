...
class SequenceFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Apply ORB augmentation
        tgt_img = extract_orb(tgt_img)
        ref_imgs = [extract_orb(ref_img) for ref_img in ref_imgs]

        # Convert to float32 (after ORB to avoid interfering with feature detection)
        tgt_img = tgt_img.astype(np.float32)
        ref_imgs = [img.astype(np.float32) for img in ref_imgs]
