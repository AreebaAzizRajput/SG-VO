...
class Normalize(object):
    ...
    def __call__(self, images, intrinsics):
        for tensor in images: # 3 images(2 reference 1 tgt)
            rgb_tensor = tensor[:3]
            for t, m, s in zip(rgb_tensor, self.mean, self.std):
                t.sub_(m).div_(s)
        return images, intrinsics
...
class RandomScaleCrop(object):
    ...
    def __call__(self, images, intrinsics):
        ...
        scaled_images = [cv2.resize(im, (scaled_w, scaled_h), interpolation=cv2.INTER_NEAREST) for im in images]
